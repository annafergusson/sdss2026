...
It was written to support our talk at SDSS 2026 titled *Learning through tinkering: Designing interactive worked examples for introductory-level data science students*. The [two-page extended abstract for the talk is available here](https://raw.githubusercontent.com/annafergusson/sdss2026/main/sdss26_extended_abstract.pdf) and the [slides from the talk can be found here](https://raw.githubusercontent.com/annafergusson/sdss2026/main/sdss26_slides.pdf).
...
This is a modified version of the lab task used, as some aspects of the lab task link to material/activities covered in the lectures before the lab task. Our lab tasks also provide interactive "tinker questions" ([see Fergusson & Pfannkuch, 2022](https://iase-pub.org/ojs/SERJ/article/download/49/333/2197)) which are not included in this version.

This version of the lab task was created using [Quarto](https://github.com/annafergusson/sdss2026/blob/main/index.qmd) and the Quarto extension [`{code-blanks}`](https://github.com/annafergusson/code-blanks).
...
get_nasa_weather_data <- function(location, start_date = NULL, end_date = format(Sys.Date() - 1, "%Y-%m-%d")) {
  ...
  data <- map_df(locations, function(loc) {
    url <- paste0("https://datalandscapes.online/scripts/nasa_weather_data.php?location=", loc,"&start_date=", start_date, "&end_date=", end_date) %>% URLencode()
    tryCatch({
      result <- fromJSON(url)
      if (length(result) == 0) {
        return(tibble())
